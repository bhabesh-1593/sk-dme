...
/**
 * Performance test for the SalesOrderDocuments entity.
 */
class SalesOrderDocumentsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SalesOrderDocuments entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all salesOrderDocuments")
            .get("/services/salesorder/api/sales-order-documents")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new salesOrderDocuments")
            .post("/services/salesorder/api/sales-order-documents")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "patientId":"0"
                , "patientName":"SAMPLE_TEXT"
                , "patientDob":"2020-01-01T00:00:00.000Z"
                , "patientDod":"2020-01-01T00:00:00.000Z"
                , "patientSsn":"SAMPLE_TEXT"
                , "qmbStatus":"SAMPLE_TEXT"
                ...
                , "scanBy":"0"
                , "fileUploadPath":"SAMPLE_TEXT"
                , "uploadDate":"2020-01-01T00:00:00.000Z"
                , "uploadBy":"0"
                , "scanDate":"2020-01-01T00:00:00.000Z"
                , "reviewStatus":"SAMPLE_TEXT"
                , "reviewReason":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_salesOrderDocuments_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created salesOrderDocuments")
                .get("/services/salesorder${new_salesOrderDocuments_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created salesOrderDocuments")
            .delete("/services/salesorder${new_salesOrderDocuments_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
